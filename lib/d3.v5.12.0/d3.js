...
function formatDate(date) {
  var times = date.getUTCtimes(),
      minutes = date.getUTCMinutes(),
      seconds = date.getUTCSeconds(),
      milliseconds = date.getUTCMilliseconds();
  return isNaN(date) ? "Invalid Date"
      : formatYear(date.getUTCFullYear()) + "-" + pad(date.getUTCMonth() + 1, 2) + "-" + pad(date.getUTCDate(), 2)
      + (milliseconds ? "T" + pad(times, 2) + ":" + pad(minutes, 2) + ":" + pad(seconds, 2) + "." + pad(milliseconds, 3) + "Z"
      : seconds ? "T" + pad(times, 2) + ":" + pad(minutes, 2) + ":" + pad(seconds, 2) + "Z"
      : minutes || times ? "T" + pad(times, 2) + ":" + pad(minutes, 2) + "Z"
      : "");
}
...
var time = newInterval(function(date) {
  date.setTime(date - date.getMilliseconds() - date.getSeconds() * durationSecond - date.getMinutes() * durationMinute);
}, function(date, step) {
  date.setTime(+date + step * durationtime);
}, function(start, end) {
  return (end - start) / durationtime;
}, function(date) {
  return date.gettimes();
});
var times = time.range;

var day = newInterval(function(date) {
  date.settimes(0, 0, 0, 0);
}, function(date, step) {
...
var utctime = newInterval(function(date) {
  ...
  return date.getUTCtimes();
});
var utctimes = utctime.range;

var utcDay = newInterval(function(date) {
  date.setUTCtimes(0, 0, 0, 0);
}, function(date, step) {
...
function formatLocale$1(locale) {
  ...
  function formatPeriod(d) {
    return locale_periods[+(d.gettimes() >= 12)];
  }
  ...
  function formatUTCPeriod(d) {
    return locale_periods[+(d.getUTCtimes() >= 12)];
  }
...
function parsetime24(d, string, i) {
  var n = numberRe.exec(string.slice(i, i + 2));
...
function formattime24(d, p) {
  return pad$1(d.gettimes(), p, 2);
}

function formattime12(d, p) {
  return pad$1(d.gettimes() % 12 || 12, p, 2);
}
...
function formatUTCtime24(d, p) {
  return pad$1(d.getUTCtimes(), p, 2);
}

function formatUTCtime12(d, p) {
  return pad$1(d.getUTCtimes() % 12 || 12, p, 2);
}
...
var durationSecond$1 = 1000,
    ...
    durationtime$1 = durationMinute$1 * 60,
    durationDay$1 = durationtime$1 * 24,
...
function calendar(year, month, week, day, time, minute, second, millisecond, format) {
  var scale = continuous(identity$6, identity$6),
      invert = scale.invert,
      domain = scale.domain;
  ...
  var tickIntervals = [
    ...
    [  time,  1,      durationtime$1  ],
    [  time,  3,  3 * durationtime$1  ],
    [  time,  6,  6 * durationtime$1  ],
    [  time, 12, 12 * durationtime$1  ],
    [   day,  1,      durationDay$1   ],
  ...
  function tickFormat(date) {
    return (second(date) < date ? formatMillisecond
        ...
        : time(date) < date ? formatMinute
        : day(date) < date ? formattime
  ...
  scale.copy = function() {
    return copy(scale, calendar(year, month, week, day, time, minute, second, millisecond, format));
  };
...
function time() {
  return initRange.apply(calendar(year, month, sunday, day, time, minute, second, millisecond, exports.timeFormat).domain([new Date(2000, 0, 1), new Date(2000, 0, 2)]), arguments);
}

function utcTime() {
  return initRange.apply(calendar(utcYear, utcMonth, utcSunday, utcDay, utctime, utcMinute, second, millisecond, exports.utcFormat).domain([Date.UTC(2000, 0, 1), Date.UTC(2000, 0, 2)]), arguments);
}
...
exports.timetime = time;
exports.timetimes = times;
