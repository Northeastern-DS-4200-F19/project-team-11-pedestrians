...
var state = {
  ...
  view: "crime",
  neighborhood: [],
  data: { crime: null, demographic: null, real_estate: null, survey: null },
  set setN(x) {
  ...
  colors: {
    ...
    real_estate: "green",
    demographic: "blue",
    chester_square: "orange"
  },
  set setView(x) {
    this.view = x;
    //updateViz();
    const linedata = filterLine(state.data[state.view]);
    removeChart();
    lineChart(linedata);
    ...
    tablevis(
      ...
      state.data[state.view]
        .filter(a => a.neighborhood == "Chester Square")
        .map(s => s.value),
      new Set(state.data[state.view].map(s => s.neighborhood))
    );
  }
};
...
const filterBar = d => {
  ...
  d.forEach(obj => {
    result[obj.neighborhood][obj.category] =
      result[obj.neighborhood][obj.category] + obj.value;
  });
...
const filterLine = d => {
  ...
  if (state.view == "demographic") {
    ...
    return d
      .map(item => {
        return {
          time: new Date(Date.parse(item.time)).getFullYear(),
          value: item.value,
...
const render = () => {
  ...
  csOverTime({
    ...
    actual: linedata.filter(
      a => a.neighborhood == "Chester Square"
    ),
    perceived: transformSurvey(state.data["survey"])
  });
};
